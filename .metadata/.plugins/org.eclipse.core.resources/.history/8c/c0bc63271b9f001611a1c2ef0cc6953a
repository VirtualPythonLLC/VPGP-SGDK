...
void SetupForegroundLayer()
{
    ...
    for (loop = starttiley; loop <= endtiley; loop++)
    {
        ...
        for (innerLoop = starttilex; innerLoop <= endtilex; innerLoop++)
        {
            ...
            VDP_setTileMap(APLAN, tileNumber, innerLoop & 63, loop & 63);
        }
    }
}
...
void ClearCoin(u16 tileindex, u16 tilex, u16 tiley)
{
	...
	VDP_setTileMap(APLAN, tileattr, tilex, tiley);
	VDP_setTileMap(APLAN, tileattr, (tilex + 1)& 63, tiley);
	VDP_setTileMap(APLAN, tileattr, tilex, (tiley + 1) & 63);
	VDP_setTileMap(APLAN, tileattr, (tilex + 1) & 63, (tiley + 1) & 63);
}
...
void LevelManager_DrawTilePlanes()
{
    ...
    VDP_setVerticalScroll(APLAN, scrollData.scrolly_vdp);
    VDP_setVerticalScroll(BPLAN, scrollData.scrolly_vdp >> 2);
