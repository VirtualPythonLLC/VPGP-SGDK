...
Sprite* healthBarTopSprite;
Sprite* healthBarBottomSprite;
...
void DrawIcon(ObjectStruct* enemyObject)
{
    s16 tile_distance_x = VIRTUAL_TO_TILE(-scrollData.scrollx_hires_clamped + PIXEL_TO_VIRTUAL(160) - enemyObject->x);
    s16 tile_distance_y = VIRTUAL_TO_TILE(scrollData.scrolly_hires_clamped + PIXEL_TO_VIRTUAL(124) - enemyObject->y);
...
{
    // Draw player icon.
	playerIconSprite.index = spriteDrawIndex + 1;
    VDP_setSpriteP(spriteDrawIndex, &playerIconSprite);
    spriteDrawIndex++;

    // Draw coin icon.
	coinIconSprite.index = spriteDrawIndex + 1;
    VDP_setSpriteP(spriteDrawIndex, &coinIconSprite);
    spriteDrawIndex++;

	// Draw Health Bar (two parts)
	healthBarTopSprite.index = spriteDrawIndex + 1;
    VDP_setSpriteP(spriteDrawIndex, &healthBarTopSprite);
    spriteDrawIndex++;

	healthBarBottomSprite.index = spriteDrawIndex + 1;
...
void InitHUD()
{
	...
	// init player icon.
	playerIconSprite.x = 10;
	playerIconSprite.y = 188;
	playerIconSprite.attribut = TILE_ATTR_FULL(PAL0, 1, 0, 0, playerLoadedTileInfo.startIndex);
	playerIconSprite.lastVDPSprite.size = SPRITE_SIZE(3,3);

	// init coins icon.
	coinIconSprite.x = 294;
	coinIconSprite.y = 190;
	coinIconSprite.attribut = TILE_ATTR_FULL(PAL0, 1, 0, 0, coinLoadedTileInfo.startIndex);
	coinIconSprite.lastVDPSprite.size = SPRITE_SIZE(2,2);

	// init health bar.
	healthBarTopSprite.x = 10;
	healthBarTopSprite.y = 10;
	healthBarTopSprite.lastVDPSprite.size = SPRITE_SIZE(1,4);
	healthBarBottomSprite.x = 10;
	healthBarBottomSprite.y = 42;
	healthBarBottomSprite.lastVDPSprite.size = SPRITE_SIZE(1,4);
...
{
	healthBarTopSprite.attribut = TILE_ATTR_FULL(PAL0, 1, 0, 0, heartLoadedTileInfo.startIndex + healthToSpriteIndexTop[playerObject->healthPoints]);
	healthBarBottomSprite.attribut = TILE_ATTR_FULL(PAL0, 1, 0, 0, heartLoadedTileInfo.startIndex + healthToSpriteIndexBottom[playerObject->healthPoints]);
}
