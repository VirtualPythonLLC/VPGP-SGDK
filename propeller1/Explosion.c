...
u16 ExplosionCreate(u16 x, u16 y)
{
    ...
    explosionObject->x                  = x;
    explosionObject->y                  = y;
    explosionObject->sprite.lastVDPSprite.size        = SPRITE_SIZE(2,2);
...
void ExplosionUpdate(ObjectStruct* explosionObject)
{
    ...
    explosionObject->sprite.x = VIRTUAL_TO_PIXEL(explosionObject->x) + scrollData.scrollx_vdp - 8;
    explosionObject->sprite.y = VIRTUAL_TO_PIXEL(explosionObject->y) - scrollData.scrolly_vdp - 8;
...
{
    explosionObject->sprite.index  = spriteDrawIndex + 1;
    explosionObject->sprite.attribut  = TILE_ATTR_FULL(PAL0,
                                                        1,
