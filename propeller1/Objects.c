...
void ResetObjectValues(ObjectStruct* object)
{
    object->sprite.x         = -64;
    object->sprite.y         = -64;
    object->sprite.attribut    = (u16)-1;
    object->sprite.lastVDPSprite.size         = (u8)-1;
    object->sprite.index         = (u8)-1;
    ...
    for (; loop < objectCollection->maxActiveObjects; loop++)
    {
        ...
        else if (object->objectState == OBJECTSTATE_ACTIVE)
        {
            ...
            if (object->objectState == OBJECTSTATE_ACTIVE)
            {
                ...
                object->visible = SpriteIsOnScreen(object->sprite.x, object->sprite.y, 32);
