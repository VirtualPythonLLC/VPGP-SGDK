...
void PlayerUpdate(ObjectStruct* playerObject)
{
    ...
    playerObject->sprite.x = (s16)VIRTUAL_TO_PIXEL(playerObject->x + scrollData.scrollx_hires_clamped) - 12;
    playerObject->sprite.y = (s16)VIRTUAL_TO_PIXEL(playerObject->y - scrollData.scrolly_hires_clamped) - 12;
}
...
{
    playerObject->sprite.attribut = TILE_ATTR_FULL(PAL0,0,0,0,0) + playerLoadedTileInfo.startIndex;

    playerObject->sprite.index  = spriteDrawIndex + 1;
...
void PlayerCreate()
{
    ...
    playerObject->sprite.lastVDPSprite.size       = SPRITE_SIZE(3,3);
