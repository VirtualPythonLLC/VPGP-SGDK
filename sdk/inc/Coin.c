...
u16 CoinCreate(s16 x, s16 y)
{
    ...
    coinObject->speedx            = 0;
    coinObject->speedy            = 0;
    coinObject->sprite.lastVDPSprite.size       = SPRITE_SIZE(2,2);
...
void CoinUpdate(ObjectStruct* coinObject)
{
    ...
    coinObject->sprite.x = VIRTUAL_TO_PIXEL(coinObject->x) + scrollData.scrollx_vdp;
    coinObject->sprite.y = VIRTUAL_TO_PIXEL(coinObject->y) - scrollData.scrolly_vdp;
    coinObject->sprite.attribut = coinAnimationFrames[coinObject->spriteIndex >> 3] + coinLoadedTileInfo.startIndex;
}
...
{
    coinObject->sprite.index  = spriteDrawIndex + 1;
