...
u16 BulletCreate(s16 x, s16 y, s16 speedx, s16 speedy)
{
    ...
    bulletObject->sprite.lastVDPSprite.size       = SPRITE_SIZE(1,1);
    bulletObject->spriteIndex       = 0;
    bulletObject->sprite.attribut  = TILE_ATTR_FULL(PAL0,1,0,0,bulletLoadedTileInfo.startIndex);
...
void BulletUpdate(ObjectStruct* bulletObject)
{
    ...
    bulletObject->sprite.x = VIRTUAL_TO_PIXEL(bulletObject->x) + scrollData.scrollx_vdp - 4;
    bulletObject->sprite.y = VIRTUAL_TO_PIXEL(bulletObject->y) - scrollData.scrolly_vdp - 4;
...
{
    bulletObject->sprite.index  = spriteDrawIndex + 1;
