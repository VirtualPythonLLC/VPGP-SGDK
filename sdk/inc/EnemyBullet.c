...
u16 EnemyBulletCreate(s16 x, s16 y, s16 speedx, s16 speedy)
{
    ...
    enemyBulletObject->sprite.lastVDPSprite.size       = SPRITE_SIZE(1,1);
    enemyBulletObject->spriteIndex       = 0;
    enemyBulletObject->sprite.attribut  = TILE_ATTR_FULL(PAL0,1,0,0,bulletLoadedTileInfo.startIndex);
...
void EnemyBulletUpdate(ObjectStruct* enemyBulletObject)
{
    ...
    enemyBulletObject->sprite.x = VIRTUAL_TO_PIXEL(enemyBulletObject->x) + scrollData.scrollx_vdp - 4;
    enemyBulletObject->sprite.y = VIRTUAL_TO_PIXEL(enemyBulletObject->y) - scrollData.scrolly_vdp - 4;
...
{
    enemyBulletObject->sprite.index  = spriteDrawIndex + 1;
