...
u16 EnemyCreate(s16 x, s16 y)
{
    ...
    enemyObject->sprite.lastVDPSprite.size       = SPRITE_SIZE(3,3);
    enemyObject->spriteIndex       = random() & 31;
    enemyObject->sprite.attribut  = enemyAnimationFrames[enemyObject->spriteIndex] + enemyLoadedTileInfo.startIndex;
...
void EnemyUpdate(ObjectStruct* enemyObject)
{
    ...
    enemyObject->sprite.x = VIRTUAL_TO_PIXEL(enemyObject->x) + scrollData.scrollx_vdp - 12;
    enemyObject->sprite.y = VIRTUAL_TO_PIXEL(enemyObject->y) - scrollData.scrolly_vdp - 12;
...
{
    enemyObject->sprite.index  = spriteDrawIndex + 1;
