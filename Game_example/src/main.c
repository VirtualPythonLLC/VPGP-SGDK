...
int main()
{
   ...
   // SND_setPCM_XGM(SFX_ROLL, sonic_roll_sfx, sizeof(sonic_roll_sfx));
    ...
    // init sonic sprite
    sprites[0] = SPR_addSprite(&cody_sprite, fix32ToInt(posx - camposx), fix32ToInt(posy + camposy), TILE_ATTR(PAL2, TRUE, FALSE, FALSE));
    ...
    memcpy(&palette[32], cody_sprite.palette->data, 16 * 2);
...
static void updatePhysic()
{
    ...
    u16 c;
	for( c = 1; c < NUM_SPRITES; c++) {
	   if(doOverlap(sprites[0], sprites[c])) {
		  // Collided
